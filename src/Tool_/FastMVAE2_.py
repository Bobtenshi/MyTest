...
from Tools.stft_or import spectrogram, power
...
def FastMVAE2(X, hydra):
    ...
    for it in range(hydra.const.iteration):
        ...
        """
        wif it%10 == 0 or it==29:
            fig = plt.figure()
            label = lhat.to('cpu').detach().numpy().copy()
            left = np.arange(label.shape[1])
            for idx in range(label.shape[0]):
                plt.plot(left,label[idx,:], color=color_palette[idx])
            plt.xlabel("label dim")
            plt.ylabel("label value")
            plt.grid(True)
            plt.savefig( f"est_label_itr{it}.png")
        """
        ...
        Rhat[:, 0, :] = R[:, 0, :]
        R = Rhat.transpose(1, 2, 0)  # (freq, time, n_src)
        ...
        if USE_NORM:
            W_n, R, P = local_normalize(W_n, R, P, I, J)

        cost_updW_list.append(cost_function(X, R.transpose([2, 0, 1]), W_n))
